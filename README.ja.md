...
**[Windows Beta 1をダウンロード（x64 ZIP）](https://github.com/jydie5/Trimlet/releases/download/v0.4.0-beta.1/Trimlet-0.4.0-beta.1-win-x64.zip)** · [起動方法と制限](apps/windows/BETA_DOWNLOAD.md)。署名なしのポータブルベータ版です。.NETは同梱、FFmpegは別途必要です。このリリースにMacのバイナリは含みません。

## アプリの作業画面

![Windows版Trimlet：動画プレビュー、独立したIN／OUT、名前付きクリップ、プロジェクト保存と書き出し](docs/images/windows-workspace-2026-09.png)

_自作の合成デモ動画を使用した、実行中のWindows版のスクリーンショットです。動画から切り出し、並べて書き出すまでの作業画面を紹介しています。[デモ素材の生成手順](docs/WINDOWS_DEMO.md)（説明ページです。動画へのリンクではありません）。_
...
- Windows：`v0.4.0-beta.1`として、ランタイム同梱・署名なしのx64ポータブルZIPを配布します。プロジェクト保存・再開とビルド10のタイムライン操作を追加しました。停止中の範囲設定、独立したつまみ、ズーム、保存・再開、終了時の未保存確認は開発者が実アプリで確認済みです。包括的なヒューマンチェックは別途必要です。
- 同等性：共通プロジェクト形式v1と改訂された操作仕様は両OSに実装済みです。Windowsの実フレーム時刻による移動は維持しています。署名、クリーン環境と幅広い実動画でのリリース検証は引き続きOS別の作業です。
- macOS最新版：[v0.3.0-beta.1](https://github.com/jydie5/Trimlet/releases/tag/v0.3.0-beta.1)をMIT Licenseのソースのみで公開しています。

Windows Beta：[v0.4.0-beta.1](https://github.com/jydie5/Trimlet/releases/tag/v0.4.0-beta.1)（x64ポータブルバイナリ）。過去のEarly Accessはソースのみの配布です。

ソースツリーにFFmpeg、ffprobe、テスト動画、生成済みアプリは含みません。Windowsのリリース添付にはアプリとランタイムを含みますが、FFmpeg／ffprobeと動画は含みません。FFmpegは別途必要です。
